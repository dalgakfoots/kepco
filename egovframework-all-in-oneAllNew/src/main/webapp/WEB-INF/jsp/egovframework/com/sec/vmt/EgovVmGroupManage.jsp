...
function fncSelectGroupList(pageNo){
    ...
    document.listForm.action = "<c:url value='/sec/vmt/EgovVmGroupList.do'/>";
    document.listForm.submit();
}

function fncSelectGroup(groupId) {
    document.listForm.groupId.value = groupId;
    document.listForm.action = "<c:url value='/sec/vmt/EgovVmGroup.do'/>";
    document.listForm.submit();
}

function fncAddGroupInsert() {
    location.replace("<c:url value='/sec/vmt/EgovVmGroupInsertView.do'/>");
}

function fncGroupListDelete() {
	if(fncManageChecked()) {
	    if(confirm("<spring:message code="comCopSecGmt.validate.confirm.delete" />")) {//삭제하시겠습니까?
            document.listForm.action = "<c:url value='/sec/vmt/EgovVmGroupListDelete.do'/>";
            document.listForm.submit();
	    }
	}
}
...
function linkPage(pageNo){
    document.listForm.searchCondition.value = "1";
    document.listForm.pageIndex.value = pageNo;
    document.listForm.action = "<c:url value='/sec/vmt/EgovVvGroupList.do'/>";
    document.listForm.submit();
}
...
<form:form name="listForm" action="${pageContext.request.contextPath}/sec/vmt/EgovVmGroupList.do" method="post">
<div class="board">
	<h1>KEPCO VM 그룹 <spring:message code="title.list" /></h1><!-- 그룹관리 목록 -->
	<!-- 검색영역 -->
	<div class="search_box" title="<spring:message code="common.searchCondition.msg" />">
		<ul>
			<li><div style="line-height:4px;">&nbsp;</div><div><%-- <spring:message code="comCopSecGmt.searchCondition.searchKeywordText" /> --%>그룹 명 : </div></li><!-- 그룹 명 -->
			<!-- 검색키워드 및 조회버튼 -->
			<li>
				...
				<span class="btn_b"><a href="<c:url value='/sec/vmt/EgovVmGroupInsertView.do'/>" onClick="javascript:fncAddGroupInsert();"  title="<spring:message code="button.create" /> <spring:message code="input.button" />"><spring:message code="button.create" /></a></span><!-- 등록 -->
			</li>
		</ul>
	</div>
	...
	<tr>
		...
		<th class="board_th_link"><%-- <spring:message code="comCopSecGmt.list.groupId" /> --%> VM 그룹ID</th><!-- 그룹 ID -->
		<th><%-- <spring:message code="comCopSecGmt.list.groupNm" /> --%> VM 그룹명</th><!-- 그룹 명 -->
	...
	<tr>
	    ...
	    <td><a href="<c:url value='/sec/vmt/EgovVmGroup.do?groupId=${group.groupId}'/>" onclick="javascript:fncSelectGroup('<c:out value="${group.groupId}"/>')"><c:out value="${group.groupId}"/></a></td>
	    <td><c:out value="${group.groupNm}"/></td>
	    <%-- <td><c:out value="${group.groupDc}"/></td> --%>
	    <td><c:out value="${fn:substring(group.groupCreatDe,0,10)}"/></td>
	    <td><a href="<c:url value='/sec/vmt/EgovVmGroup.do?groupId=${group.groupId}'/>" onclick="javascript:fncSelectGroup('<c:out value="${group.groupId}"/>')"><img src="<c:url value='/images/egovframework/com/cmm/btn/btn_search.gif'/>"  align="middle" alt="<spring:message code="title.detail" />"  title="<spring:message code="title.detail" />"></a></td>
	</tr>
	</c:forEach>
	</tbody>
	</table>
