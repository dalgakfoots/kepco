...
function fnAddUserView(){
	...
	for(var i = 0 ; i < chkList.length ; i++){
		if (chkList[i].checked == true){
			...
			var temp = i + 1;
			var uniqId = document.querySelector("#board > tbody > tr:nth-child("+ (temp) +") > td:nth-child(2) > input[type=hidden]:nth-child(2)").value;
			newObj.uniqId = uniqId.split(':')[1];
			var mberId = document.querySelector("#board > tbody > tr:nth-child("+ (temp) +") > td:nth-child(3) > a").innerText;
			newObj.mberId = mberId;
			var mberNm = document.querySelector("#board > tbody > tr:nth-child("+ (temp) +") > td:nth-child(4)").innerText;
			newObj.mberNm = mberNm;
			list.push(newObj);
...
<table id="board" class="board_list" summary="<spring:message code="common.summary.list" arguments="${pageTitle}" />">
